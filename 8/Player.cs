...
public class Player : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        //Breakable
        BoxCollider2D c = GetComponent<BoxCollider2D>();
        if (obj.GetComponent<BreakableBricks>() != null && rb.velocity.y == 0 &&
            obj.transform.position.y - obj.GetComponent<Collider2D>().bounds.size.y / 2 >= this.transform.position.y)
        {
            GameObject.Destroy(collision.gameObject);
            BreakSE.Play();
            if (obj.GetComponent<BreakableBricks>().ID == 1)
            {
                Instantiate(BrickParticle1, obj.transform.position, Quaternion.identity).GetComponent<ParticleSystem>().Play();
            }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            GameoverPanel.SetActive(true);
            GameoverText.text = "Stage Completed!";
            Time.timeScale = 0;
        }
    }
}